...
with open(os.path.join(path_out,'COPY_FIRST_if_code_is_running10.txt'),"w") as interim:
	...
	for i,b in enumerate(bounds):
		if bounds[i]!=bounds[-1]:
			...
			for var in dependent_variables:	
				print "var 9 is now at dependent variable: %s" %str(var)
				...
				for size in range(len(feature_groups)+1):
					for set in combinations(feature_groups,size):
						if set != ():
							#print "var 9 is now making the frame for feature group %s" %str(set)
			...
			with open(os.path.join(path_out,'variable_combinations_results_balanced10.p'), 'w') as file:
				file.write(pickle.dumps(variable_combinations_results_balanced))
			with open(os.path.join(path_out,'variable_combinations_results_balanced10.txt'), 'w') as file:
				file.write(str(variable_combinations_results_balanced))
			
			with open(os.path.join(path_out,'variable_combinations_results_unbalanced10.p'), 'w') as file:
				file.write(pickle.dumps(variable_combinations_results_unbalanced))
			with open(os.path.join(path_out,'variable_combinations_results_unbalanced10.txt'), 'w') as file:
				file.write(str(variable_combinations_results_unbalanced))
			# en nu naar de volgende fold
		# en wanneer je alle folds gehad hebt:
		else:
			print "that's 10, the last!!"
