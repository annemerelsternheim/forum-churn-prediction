...
from sklearn.metrics import roc_auc_score, f1_score, confusion_matrix # for determining the predictive accuracy of the model
...
def keywithmaxval(dictionary):
	""" create a list of the dict's keys and values and return the key with the max value"""  
...
with open(os.path.join(path_out,'COPY_FIRST_if_code_is_running_cm.txt'),"w") as interim:
	interim.write("B/U;dependent;independent;AUC;eta;depth;cm(tn,fp,fn,tp)\n")
	...
	for i,b in enumerate(bounds):
		if bounds[i]!=bounds[-1]:
			...
			for var in dependent_variables:	
				print "doing churn in %s month(s)"%str(var+1)
				...
				print "data got balanced/unbalanced accordingly.."
				...
				print "y was split from the rest.."
				...
				for size in range(len(feature_groups)+1):
					for set in combinations(feature_groups,size):
						if set != ():
							print "let's take a look at feature group combination %s"%str(set)
							...
							print "df were initiated for take-off (a.k.a: we start assembling X now)"
							for tuple in set:
							...
							for parameter_settings in [(0.05,4),(0.05,6),(0.05,8),(0.1,4),(0.1,6),(0.1,8),(0.2,4),(0.2,6),(0.2,8)]:
								print "with this X and earlier gotten y, we try parameter settings %s"%str(parameter_settings)
							...
							print "best classifier determined. now initialise it"
							...
							tn_b, fp_b, fn_b, tp_b = confusion_matrix(test_data_balanced_y,y_val_balanced).ravel() 
							variable_combinations_results_balanced[(var,set)].append((auc_balanced, best_parameter_settings_balanced, (tn_b, fp_b, fn_b, tp_b)))
							#interim.write("B;%s;%s;%s;%s;%s;%s\n" %(str(var),str(set),str(auc_balanced),str(best_parameter_settings_balanced[0]),str(best_parameter_settings_balanced[1]),str((tn_b, fp_b, fn_b, tp_b))))
							print "\t predict for unbalanced"
							...
							print "\t confusion matrix"
							tn_u, fp_u, fn_u, tp_u = confusion_matrix(test_data_unbalanced_y,y_val_unbalanced).ravel()
							print "and save all scores\n"
							variable_combinations_results_unbalanced[(var,set)].append((auc_unbalanced, best_parameter_settings_unbalanced, (tn_u, fp_u, fn_u, tp_u)))
							#interim.write("B;%s;%s;%s;%s;%s;%s\n" %(str(var),str(set),str(auc_unbalanced),str(best_parameter_settings_unbalanced[0]),str(best_parameter_settings_unbalanced[1]),str((tn_u, fp_u, fn_u, tp_u))))
			
			# write to file in-between
			with open(os.path.join(path_out,'variable_combinations_results_balanced_cm.p'), 'w') as file:
				file.write(pickle.dumps(variable_combinations_results_balanced))
			with open(os.path.join(path_out,'variable_combinations_results_balanced_cm.txt'), 'w') as file:
				file.write(str(variable_combinations_results_balanced))
			
			with open(os.path.join(path_out,'variable_combinations_results_unbalanced_cm.p'), 'w') as file:
				file.write(pickle.dumps(variable_combinations_results_unbalanced))
			with open(os.path.join(path_out,'variable_combinations_results_unbalanced_cm.txt'), 'w') as file:
				file.write(str(variable_combinations_results_unbalanced))
			print "done with fold %i. Took this long: %s" %(i,str(datetime.now()-start_fold))
			# en nu naar de volgende fold
		# en wanneer je alle folds gehad hebt:
		else:
			print "that's it! Total time passed since start: %s" %str(datetime.now()-start)
