...
with open(os.path.join(path_out,'COPY_FIRST_if_code_is_running_unbalanced_train.txt'),"w") as interim:
	interim.write("B/U;dependent;independent;AUC;eta;depth;f1;cm\n")
	...
	for i,b in enumerate(bounds):
		if bounds[i]!=bounds[-1]:
			...
			for var in dependent_variables:			
				...
				train_data_unbalanced = train_data
				...
				train_data_unbalanced_y = train_data_unbalanced[var]
				...
				for size in range(len(feature_groups)+1):
					for set in combinations(feature_groups,size):
						if set != ():
							...
							train_data_unbalanced_X = pd.DataFrame()
							...
							for tuple in set:
								for feature in tuple:
									...
									train_data_unbalanced_X[feature] = train_data_unbalanced[feature]
							...
							for (eta,depth) in [(eta,depth) for eta in [0.05,0.1,0.2]
								...
								classifier.fit(train_data_unbalanced_X,train_data_unbalanced_y,eval_metric="auc")
							...
							best_classifier_unbalanced.fit(train_data_unbalanced_X,train_data_unbalanced_y,eval_metric="auc")
							...
							#variable_combinations_results_balanced[(var,set)].append((auc_balanced, best_parameter_settings_balanced, f1_balanced, cm_b))
							#interim.write("B;%s;%s;%s;%s;%s;%s;%s\n" %(str(var),str(set),str(auc_balanced),str(best_parameter_settings_balanced[0]),str(best_parameter_settings_balanced[1]),str(f1_balanced),str(cm_b)))
							...
							variable_combinations_results_unbalanced[(var,set)].append((auc_unbalanced, best_parameter_settings_unbalanced, f1_unbalanced, cm_u))
							interim.write("B;%s;%s;%s;%s;%s;%s;%s\n" %(str(var),str(set),str(auc_unbalanced),str(best_parameter_settings_unbalanced[0]),str(best_parameter_settings_unbalanced[1]),str(f1_unbalanced),str(cm_u)))
			
			# write to file in-between
			# with open(os.path.join(path_out,'variable_combinations_results_balanced_5.p'), 'w') as file:
				# file.write(pickle.dumps(variable_combinations_results_balanced))
			# with open(os.path.join(path_out,'variable_combinations_results_balanced_%.txt'), 'w') as file:
				# file.write(str(variable_combinations_results_balanced))
			
			with open(os.path.join(path_out,'variable_combinations_results_unbalanced_5.p'), 'w') as file:
				file.write(pickle.dumps(variable_combinations_results_unbalanced_train))
			with open(os.path.join(path_out,'variable_combinations_results_unbalanced_5.txt'), 'w') as file:
				file.write(str(variable_combinations_results_unbalanced_train))
