...
from sklearn.metrics import mean_absolute_error, roc_auc_score, f1_score # for determining the predictive accuracy of the model
...
#data = pd.DataFrame({"y": [1,2,3,4,5,6,7,8,9],"A": [1,2,3,4,5,6,7,8,9]})
...
clf = xgb.XGBClassifier(max_depth=6,min_child_weight=1,learning_rate=.05,gamma=.01, n_estimators = 800 , njobs = -1, objective = 'binary:logistic', silent = False)
...
# with open(os.path.join(path_out,'COPY_FIRST_if_code_is_running2.txt'),"w") as interim:
	...
	# for i,b in enumerate(bounds):
		# if bounds[i]!=bounds[-1]:
			...
			# # with open(os.path.join(path_out,'test1.p'), 'w') as file:
				# # file.write(pickle.dumps(variable_combinations_results_balanced))
			# # with open(os.path.join(path_out,'test1.txt'), 'w') as file:
				# # file.write(str(variable_combinations_results_balanced))
			
			# # with open(os.path.join(path_out,'variable_combinations_results_unbalanced2.p'), 'w') as file:
				# # file.write(pickle.dumps(variable_combinations_results_unbalanced))
			# # with open(os.path.join(path_out,'variable_combinations_results_unbalanced2.txt'), 'w') as file:
