...
for i,b in enumerate(bounds):
	if bounds[i]!=bounds[-1]:
				...
				for set in combinations(feature_groups,size):
					if set != ():
						...
						best_classifier_balanced = xgb.XGBClassifier(learning_rate = best_parameter_settings_balanced[0], 
															...
															n_estimators = 800 , n_jobs = -1, 
															objective = 'binary:logistic', silent = True)
						best_classifier_unbalanced = xgb.XGBClassifier(learning_rate = best_parameter_settings_balanced[0], 
															max_depth = best_parameter_settings_unbalanced[1],
															n_estimators = 800 , n_jobs = -1, 
															objective = 'binary:logistic', silent = True)
						...
						y_proba_balanced = classifier.predict_proba(test_data_balanced_X)[:,1]
						auc_balanced = roc_auc_score(test_data_balanced_y,y_proba_balanced)
						variable_combinations_results_balanced[(var,set)].append((auc_balanced, best_parameter_settings_balanced))

						# calculate AUC for unbalanced test set
						y_proba_unbalanced = classifier.predict_proba(test_data_unbalanced_X)[:,1]
						auc_unbalanced = roc_auc_score(test_data_unbalanced_y,y_proba_unbalanced)
						variable_combinations_results_unbalanced[(var,set)].append((auc_unbalanced, best_parameter_settings_unbalanced))
		...
		print "BALANCED RESULTS: ", variable_combinations_results_balanced
		...
		print "UNBALANCED RESULTS: ", variable_combinations_results_unbalanced
		print
